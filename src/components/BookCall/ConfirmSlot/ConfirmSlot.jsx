...
import { bookMeetingSlots } from '@/services/bookCall';

function ConfirmSlot({ ResetData, formData, formatDate }) {
    ...
    const confirmBooking = async () => {
        ...
        const convertedDate = formatDate(formData.date);
        const bookData = {
            name: formData.Name,
        ...
        try {
            const { res, err } = await bookMeetingSlots(bookData);
            if (res) {
                setConfirm(true)
                console.log(res.data.response.returnvalue);
                setResData(res.data.response.returnvalue);
                            ...
                            <div className={styles.slotDetailsBox}>
                                <div className={styles.sltoData}>
                                    <div>
                                        ...
                                        <p><GoClock className={styles.slotIcon} />{responseData?.start_time
                                            ? new Date(responseData.start_time).toLocaleTimeString('en-US', {
                                                hour: '2-digit',
                                                minute: '2-digit',
                                            })
                                            : ''}</p>
                                        <p><GoGlobe className={styles.slotIcon} />{responseData?.customer_booking_time_zone}</p>
                                    </div>
                                </div>
